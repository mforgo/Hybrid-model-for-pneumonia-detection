...
Pro maximalizaci diskriminační schopnosti byla použita parametrizovaná měřící báze místo pevného Pauli-Z. Rotace $R_Y(\theta_{ry}) \rightarrow R_Z(\theta_{rz}) \rightarrow Z$ umožňuje modelu naučit se optimální úhel pro separaci tříd v Hilbertově prostoru. Dva dodatečné parametry ($\theta_{ry}, \theta_{rz}$) jsou trénovány spolu s ostatními parametry obvodu.
...
\paragraph{Efektivita autoenkodéru:} Pro lepší zachování sémantické struktury dat byl použit variational autoencoder (VAE) s~regularizovaným latentním prostorem. NAMÍSTO deterministického MSE autoenkodéru VAE modeluje latentní prostor jako Gaussovskou distribuci $\mathcal{N}(\mu, \sigma^2)$ s~KL divergencí jako regularizačním členem (Kingma \& Welling, 2014). Toto rozšíření zajišťuje hladší latentní prostor a~lepší generalizaci.
