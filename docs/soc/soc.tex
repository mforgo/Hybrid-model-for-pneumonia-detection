...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[node distance=2.5cm, auto, scale=0.8, transform shape]
        ...
        \node (cnn) [process, right=0.5cm of aug] {CNN};
...
Vstupní rentgenové snímky nejprve procházejí fází robustní augmentace a~jsou zpracovány předtrénovanou konvoluční sítí. Ta slouží jako výkonný extraktor příznaků a~převádí vysokorozměrná obrazová data do~kompaktních vektorů. Tyto vektory jsou následně zredukovány metodou PCA na 64 komponent a~$L_2$-normalizovány, čímž vzniká finální prostor příznaků vhodný pro kvantové amplitudové kódování.
...
Pro získání smysluplných reprezentací rentgenových snímků byly zpočátku porovnávány dvě odlišné architektury z~knihovny PyTorch (\textit{torchvision}):
\begin{itemize}
    \item \textbf{Vision Transformer (ViT-B/16):} Architektura založená na mechanismu pozornosti, která snímek dělí na menší segmenty (patche).
...
Snímky byly přeškálovány a normalizovány na základě statistik datasetu ImageNet. Následně byla oběma modelům upravena poslední klasifikační vrstva tak, aby fungovaly výhradně jako extraktory. V~případě modelu ViT byla klasifikační hlava nahrazena vrstvou \texttt{nn.Identity()}, u~ConvNeXt-Tiny bylo aplikováno globální průměrování (\texttt{AdaptiveAvgPool2d}) pro redukci prostorových dimenzí a následné zploštění (\texttt{Flatten}). Oba modely tak pro každý snímek produkovaly hustý vektor o~délce 768 příznaků.
...
Jak ukazuje Obrázek \ref{fig:variance_comp}, analýza kumulativního vysvětleného rozptylu zpočátku favorizovala model ViT-B/16. Jeho příznaky se daly komprimovat mnohem efektivněji a zvolených 64 komponent u~něj pokrývalo podstatně větší část rozptylu v~datech než v~případě modelu ConvNeXt-Tiny.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{../../media/backbone_comp_2.png}
    \caption{Porovnání kumulativního vysvětleného rozptylu (PCA). Model ViT-B/16 dokázal při cílové dimenzionalitě 64 zachovat větší množství informací než ConvNeXt-Tiny.}
    \label{fig:variance_comp}
\end{figure}

\subsection{Problém s trénováním a finální volba architektury}
Ačkoliv teoretické předpoklady a analýza PCA favorizovaly model ViT-B/16, praktické experimenty s~trénováním variačního kvantového klasifikátoru (VQC) odhalily zásadní problém. Příznaky extrahované transformerovým modelem se ukázaly jako nevhodné pro optimalizaci použitého kvantového obvodu. 

VQC model se z~těchto příznaků nedokázal efektivně učit. Již po pěti epochách trénování se ukázalo, že ačkoliv hodnota chybové funkce mohla mírně klesat, metrika ROC AUC se stabilně pohybovala kolem hodnoty $0,48$. To odpovídá zcela náhodnému rozhodování klasifikátoru (tzv. random guessing) nebo dokonce mírně horšímu výkonu. Prostor příznaků z~ViT neumožnil kvantovému modelu nalézt smysluplnou rozhodovací hranici.

Z~tohoto důvodu byla jako \textbf{hlavní extrakční síť pro finální hybridní model zvolena architektura ConvNeXt-Tiny}. Přestože komprese jejích konvolučních příznaků na 64 dimenzí znamenala o~něco větší ztrátu celkového rozptylu, topologie těchto dat poskytla mnohem stabilnější základ pro amplitudové kódování a VQC se na nich dokázalo úspěšně učit.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|c|}
        ...
        F1-Score & 0,902 & 0,691 & -0,211 & [0,65; 0,73] \\
        AUC-ROC & 0,912 & 0,744 & -0,168 & [0,71; 0,78] \\
        Balanced Accuracy & 0,842 & 0,688 & -0,154 & -- \\
        \hline
