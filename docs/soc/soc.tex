...
Pro maximalizaci diskriminační schopnosti byla použita parametrizovaná měřící báze místo pevného Pauli-Z. Rotace $R_Y(\theta_{ry}) \rightarrow R_Z(theta_{rz}) \rightarrow Z)$ umožňuje modelu naučit se optimální úhel pro separaci tříd v Hilbertově prostoru. Dva dodatečné parametry (theta_ry, theta_rz) jsou trénovány spolu s ostatními parametry obvodu.
...
Experimentálně byl testován také Quantum Natural Gradient (QNG) optimalizátor využívající Fubini-Studiovu metriku místo standardní Euclidean. QNG používá přirozenou geometrii kvantového parametrického prostoru: $\theta_{k+1} = \theta_k - \eta \cdot F^{-1} \nabla \mathcal{L}$, kde $F$ je Fubini-Studiova matice. V našich experimentech však standardní Adam vykázal srovnatelnou nebo lepší konvergenci při menší výpočetní náročnosti, a proto byl ponechán jako primární optimalizátor.
