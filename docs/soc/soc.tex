...
Vektory příznaků z ResNet-50 jsou pro všechny snímky uloženy jako
\texttt{.npy} soubory spolu s metadaty v CSV souboru
\texttt{metadata.csv}, který obsahuje cestu k souboru, třídu a příslušnou
sadu (\texttt{train/val/test}). Následná redukce dimenze a normalizace
probíhá nad těmito uloženými vektory pomocí objektu
\texttt{sklearn.Pipeline}, který postupně aplikuje standardizaci
(\texttt{StandardScaler}), zvolenou metodu redukce (PCA) a \(L_2\)
normalizaci pro přípravu dat ke kvantovému kódování.
...
klasickou částí (ResNet-50 + PCA pipeline) a (ii) trénink čistě klasického
baseline klasifikátoru nad stejnými 64rozměrnými příznaky. V obou případech byl
použit optimalizační algoritmus Adam, avšak s odlišným nastavením chybových
funkcí a plánů učení reflektujících rozdílný charakter výstupu kvantového obvodu
a sigmoidového klasického modelu.
...
\begin{itemize}
  ...
  na minoritní třídě \emph{Normal} penalizovány výrazněji a částečně tak
  kompenzovaly nevyváženost dat.
  
  \item \textbf{Klasický baseline: vážená binární cross-entropy}

  Referenční vícevrstvý perceptron (baseline) produkuje pro každý vzorek
  pravděpodobnost $p_i \in [0,1]$ pomocí sigmoidové výstupní vrstvy. Pro jeho
  trénink je použita vážená binární cross-entropy
  \begin{equation}
    L_{\mathrm{BCE}} = - \frac{1}{N} \sum_{i=1}^{N}
    \bigl(
      \alpha\, y_i \log p_i
      + (1-\alpha)\,(1-y_i)\log(1-p_i)
    \bigr),
  \end{equation}
  kde $\alpha \in (0,1)$ je váhový koeficient odvozený z relativního zastoupení
  tříd tak, aby byly chyby na minoritní třídě \emph{Normal} zvýhodněny.
\end{itemize}

\paragraph{Nastavení tréninku hybridního modelu (VQC)}

Kvantová část byla trénována nad pevně extrahovanými příznaky z ResNet-50 a
PCA pipeline. Pro všechny experimenty byla použita následující sada
hyperparametrů:

\begin{itemize}
  \item počet epoch: $30$ (s \emph{early stopping} podle validační ztráty),
  \item velikost batch: $32$ vzorků,
  \item optimalizátor: Adam se základní rychlostí učení
        $\eta_0 = 10^{-3}$,
  \item minimální rychlost učení: $\eta_{\min} = 10^{-5}$,
  \item počet zahřívacích epoch (warm-up): $3$,
  \item trpělivost early stopping: $5$ epoch bez zlepšení validační ztráty.
\end{itemize}

Rychlost učení je řízena hladkým kosinovým plánem s úvodní fází
\emph{warm-up}. Po dobu prvních $E_{\mathrm{warm}} = 3$ epoch roste
$\eta(e)$ lineárně z nuly na $\eta_0$, následně je pro epochu $e$
aplikováno kosinové rozpadající se schéma mezi $\eta_0$ a $\eta_{\min}$:
\begin{equation}
  \eta(e) =
  \begin{cases}
    \displaystyle
    ...
    \displaystyle
    \eta_{\min}
    + \bigl(\eta_0 - \eta_{\min}\bigr)\,
      \frac{1 + \cos\!\bigl(\pi\,\tfrac{e-E_{\mathrm{warm}}}{E_{\mathrm{max}}-E_{\mathrm{warm}}}\bigr)}{2},
      & e > E_{\mathrm{warm}},
  \end{cases}
\end{equation}
kde $E_{\mathrm{max}} = 30$ je maximální počet epoch. Tato kombinace
rychlého náběhu a postupného kosinového útlumu se v praxi ukázala jako
stabilní kompromis mezi rychlou konvergencí a omezením přetrénování.

V každé epoše je trénovací množina náhodně promíchána a rozdělena na
mini-batche. Po zpracování celé trénovací sady je kvantový model vyhodnocen
na validační sadě a jsou sledovány metriky \emph{val loss} a \emph{val accuracy}.
Nejlepší nalezené parametry (s minimální validační ztrátou) jsou průběžně
ukládány a po splnění kritéria early stopping jsou použity pro finální
vyhodnocení na testovací sadě.

\paragraph{Nastavení tréninku klasického baseline modelu}

Klasický baseline klasifikátor nad 64rozměrnými příznaky byl trénován
odděleně, s idetickými hyperparametry (počet epoch, batch size, optimalizátor) a~váženou binární cross-entropy jako chybovou funkcí.
Stejně jako u hybridního modelu byl i zde použit early stopping na základě
vývoje validační ztráty, přičemž k finálnímu porovnání s kvantovým řešením
byla použita nejlepší konfigurace podle výsledků na validační sadě.
