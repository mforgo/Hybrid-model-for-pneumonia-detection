...
\section{Shrnutí}
\subsection{Shrnutí projektu}

V rámci maturitní práce byl navržen, implementován a otestován hybridní kvantově‑klasický model pro detekci pneumonie z rentgenových snímků hrudníku, čímž bylo naplněno zadání zaměřené na praktickou aplikaci kvantového strojového učení v medicínské diagnostice. Navržená pipeline kombinuje extrakci příznaků pomocí sítě ResNet‑50, redukci dimenze metodou PCA a variabilní kvantový obvod (VQC) s amplitudovým kódováním na 6 qubitech, který slouží jako klasifikátor s výrazně menším počtem parametrů než referenční klasický model.

Experimenty ukázaly, že čistě klasický model dosahuje vyšší přesnosti (přibližně 86~\% accuracy, F1 $\approx 0{,}90$, AUC $\approx 0{,}91$), zatímco hybridní model se pohybuje okolo 72~\% accuracy a AUC $\approx 0{,}74$, avšak s přibližně tisíckrát menším počtem trénovatelných parametrů v rozhodovací části. Přestože se nepodařilo dosáhnout srovnatelné klasifikační výkonnosti s plně klasickým ResNet‑50, projekt splnil cíl ověřit praktickou použitelnost hybridního kvantově‑klasického přístupu na reálných biomedicínských datech a identifikoval klíčová omezení současné NISQ éry (omezený počet qubitů, redukce dimenze, dataset shift mezi trénovací a testovací sadou).

\subsection{Project Summary}

This thesis designed, implemented, and evaluated a hybrid quantum‑classical model for pneumonia detection from chest X‑ray images, thus fulfilling the assignment focused on a practical application of quantum machine learning in medical diagnostics. The proposed pipeline combines feature extraction with a ResNet‑50 backbone, dimensionality reduction via PCA, and a variational quantum circuit (VQC) with amplitude encoding on 6 qubits, which acts as a classifier with a dramatically reduced number of trainable parameters compared to the reference classical model.

The experiments showed that the purely classical model achieves higher performance (about 86~\% accuracy, F1 $\approx 0.90$, AUC $\approx 0.91$), while the hybrid model reaches around 72~\% accuracy and AUC $\approx 0.74$, but with roughly a thousand times fewer trainable parameters in the decision layer. Although the original hypothesis of achieving comparable classification performance to the fully classical ResNet‑50 was not confirmed, the project met its goal of demonstrating a complete hybrid quantum‑classical pipeline on real biomedical data and highlighted key limitations of the current NISQ era (limited qubit count, feature compression, and dataset shift between training and test sets).
