...
% --- MOORE'S LAW SLIDE (Slide 2) ---
\begin{frame}{Konec Moorova zákona a motivace pro kvantové počítání}
  \begin{itemize}
    \item \textbf{Moorův zákon} (Gordon Moore, 1965): Počet tranzistorů na čipu se zdvojnásobuje přibližně každé 2 roky.
    \item Tento trend platil po několik desetiletí, ale v posledních letech \textbf{zpomaluje} v důsledku fyzikálních limitů křemíkové technologie (kvantové tunelování, tepelný disipace).
  \end{itemize}
  
  \vspace{0.3cm}
  
  \begin{block}{Důsledek}
    Další zvyšování výkonu klasických procesorů je stále obtížnější a nákladnější -- hledají se \textbf{nová výpočetní paradigmata}.
  \end{block}
  
  \vspace{0.3cm}
  
  \begin{itemize}
    \item \textbf{Kvantové počítání} nabízí alternativu: s využitím superpozice a entanglementu dokáže řešit určité třídy problémů exponenciálně rychleji než klasické počítače.
    \item Tento projekt zkoumá, zda lze kvantové klasifikátory (VQC) s minimálním počtem parametrů využít v \textbf{hybridních kvantově-klasických modelech} pro medicínskou diagnostiku -- i v době NISQ (\textit{Noisy Intermediate-Scale Quantum}).
  \end{itemize}
\end{frame}
